...
import numpy as np
from tasks.pick_place_table_wsg50 import PickPlace
...
cube_initial_orientation =  np.array([0.55, 0.55, 0.0515/2 + 0.825])
my_task = PickPlace(name="fr_pick_place_table_wsg50", 
...
task_params = my_world.get_task("fr_pick_place_table_wsg50").get_params()
...
while simulation_app.is_running():
    ...
    if my_world.is_playing():
        ...
        actions = my_controller.forward(
            ...
            end_effector_offset=np.array([0, 0, 0.270]),
            end_effector_orientation=np.array([0, 1, 0, 0]) #(w, x, y, z)
        )
